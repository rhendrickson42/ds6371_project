...
1. We ran heat map of corelation between sales price and the different numerical variables 
2. we shortlisted the  variables that has highest corelation with Sales price.
3. Then we did log transformation on all numberic variables  and again check the corelation with sales price.Then identified some more 
   varaibles that has heigh corelation with sales price.
...
11. We ran the cross validation on the traing data set  and then obtain relevant cv test score.
 ...
 * custom method choosen was Forward(select=SL)
### Conclusion
Based on above table , all the models selection is derving similar results . Any way we are going ahead with Stepwise model for deriving conclusion.
...
4. We assums there is independence.
